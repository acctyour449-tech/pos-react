  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-end sm:items-center justify-center p-0 sm:p-6">
      ...
      <motion.div initial={{ opacity: 0, y: 60 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 60 }}
        ...
        className="relative bg-white rounded-t-[2rem] sm:rounded-3xl w-full sm:max-w-lg max-h-[92vh] flex flex-col shadow-2xl overflow-hidden">
                ...
                {reviews.length === 0 ? (
                  ...
                  <div key={r.id} className="bg-gray-50 rounded-2xl p-4">
                    <div className="flex items-start justify-between mb-2">
                      <div>
                        <p className="font-bold text-sm">{r.buyer_email?.split('@')[0] || 'Người dùng'}</p>
  ...
  const submit = async () => {
    ...
    try {
      for (const item of items) {
        if (ratings[item.id]) {
          await onSubmit(item.id, ratings[item.id], comments[item.id] || '');
        }
      }
      onClose();
    } finally { setLoading(false); }
  ...
  return (
    <div className="fixed inset-0 z-[90] flex items-center justify-center p-6">
      ...
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
        className="relative bg-white rounded-3xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto">
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 flex items-center justify-between z-10 rounded-t-3xl">
          <h3 className="font-black text-lg">⭐ Đánh giá đơn hàng</h3>
        ...
        <div className="p-6 space-y-5">
          {items.map((item) => (
            <div key={item.id} className="bg-gray-50 rounded-2xl p-4 space-y-3">
              ...
              <div>
                <p className="text-xs text-gray-500 mb-1.5">Đánh giá của bạn *</p>
                <Stars value={ratings[item.id] || 0} size="lg" onChange={v => setRatings(r => ({ ...r, [item.id]: v }))} />
              </div>
              <textarea rows={2} value={comments[item.id] || ''}
                onChange={e => setComments(c => ({ ...c, [item.id]: e.target.value }))}
                placeholder="Nhận xét của bạn về sản phẩm..."
                className="w-full bg-white border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none" />
            </div>
          ))}
          <button onClick={submit} disabled={loading || Object.keys(ratings).length === 0}
            className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-black py-3.5 rounded-2xl flex items-center justify-center gap-2 transition-all">
...
export default function App() {
  ...
  // Data
  const [products, setProducts] = useState<Product[]>([]);      // seller's own
  ...
  // Seller: search in my products
  const [sellerSearch, setSellerSearch] = useState('');
  const [sellerViewMode, setSellerViewMode] = useState<'grid' | 'list'>('grid');
  const [orderFilter, setOrderFilter] = useState<string>('all'); // order status filter
  ...
  useEffect(() => {
    if (!session?.user?.id) return;
    localStorage.setItem(`hd_${session.user.id}`, JSON.stringify(hidden));
  ...
  useEffect(() => { if (session && role === 'seller' && activeTab === 'my-products') fetchMine(); }, [session, role, activeTab]);
  ...
  const updateQty = (id: number, delta: number) =>
    setCart(prev => prev.map(i => i.id !== id ? i : { ...i, quantity: Math.max(1, Math.min(i.stock ?? 999, i.quantity + delta)) }));

  const removeCart = (id: number) => { setCart(p => p.filter(i => i.id !== id)); toast('Đã xoá khỏi giỏ', 'info'); };

  const applyCoupon = () => {
    const code = couponCode.trim().toUpperCase();
    const d = COUPONS[code];
    if (d) { setCouponDiscount(d); toast(`✓ Mã "${code}" — giảm ${(d * 100).toFixed(0)}%`); }
    else toast('Mã không hợp lệ hoặc đã hết hạn', 'error');
  };
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const orderData = {
        ...
        status: 'Chờ xác nhận', shipping_address: shippingAddr.trim(),
        payment_method: paymentMethod, note: orderNote.trim() || null,
      ...
      setShippingAddr(''); setOrderNote(''); setCouponCode(''); setCouponDiscount(0); setPaymentMethod('cod');
      toast('🎉 Đặt hàng thành công! Đang chờ xác nhận...', 'success');
      setTimeout(() => setActiveTab('my-purchases'), 1500);
    } catch (e: any) { toast('Lỗi đặt hàng: ' + e.message, 'error'); }
  ...
  const updateOrderStatus = async (order: Order, newStatus: string) => {
    try {
      ...
      const msgMap: Record<string, string> = {
        ...
        'Đang giao':   `🚚 Đơn hàng #${String(order.id).slice(-6)} đang được giao!`,
        'Hoàn thành':  `🎉 Đơn hàng #${String(order.id).slice(-6)} đã hoàn thành!`,
      };
  ...
  const cancelOrder = async (order: Order) => {
    ...
    if (error) { toast(error.message, 'error'); return; }
    if (order.buyer_id) {
      await supabase.from('notifications').insert([{ user_id: order.buyer_id, order_id: order.id, message: `❌ Đơn hàng #${String(order.id).slice(-6)} đã bị huỷ.`, type: 'system', is_read: false }]);
    }
  ...
  const submitReview = async (productId: number, rating: number, comment: string) => {
    const { error } = await supabase.from('reviews').upsert([{ product_id: productId, buyer_id: session!.user.id, buyer_email: session!.user.email, rating, comment }], { onConflict: 'product_id,buyer_id' });
    if (error) throw error;
  ...
  const confirmHide = () => {
    ...
    setHidden(d => [...d, pendingHide.id]);
    setWishlist(w => w.filter(x => x !== pendingHide.id));
    if (viewProduct?.id === pendingHide.id) setViewProduct(null);
    toast('👁️ Đã ẩn sản phẩm', 'info');
    setPendingHide(null);
  };
  ...
  if (!supabaseUrl || !supabaseKey) {
    return <div className="min-h-screen flex items-center justify-center bg-red-50 p-6 text-center"><AlertCircle className="w-14 h-14 text-red-500 mx-auto mb-4" /><h1 className="text-xl font-black text-red-900">Thiếu cấu hình Supabase</h1></div>;
  }
      ...
      <AnimatePresence>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 220 }}
              ...
              {hiddenProducts.length > 0 && (
                <div className="p-4 border-t border-gray-100 flex-shrink-0">
                  <button onClick={() => { hidden.forEach(id => restore(id)); setShowHidden(false); }} className="w-full py-3 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold rounded-2xl text-sm">Hiện tất cả ({hiddenProducts.length})</button>
                </div>
              )}
            </motion.div>
          </div>
        ...
        {viewProduct && (
          <ProductModal product={viewProduct} onClose={() => setViewProduct(null)}
            onAdd={(p, qty, note) => addToCart(p, qty, note)}
            wishlisted={wishlist.includes(viewProduct.id)} onWishlist={toggleWishlist}
            reviews={productReviews} />
        )}

        {reviewOrder && (
          <ReviewModal order={reviewOrder} onClose={() => setReviewOrder(null)} onSubmit={submitReview} />
        )}
      </AnimatePresence>
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between h-16 px-5">
          <div className="flex items-center gap-6">
            ...
            <div className="hidden md:flex items-center gap-1">
              {role === 'seller' ? (
                <>
                  {[{ id: 'my-products', icon: LayoutDashboard, label: 'Sản phẩm' }, { id: 'my-orders', icon: BarChart3, label: 'Đơn hàng' }].map(({ id, icon: Icon, label }) => (
          ...
          <motion.main key="my-products" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex gap-2">
              <div className="relative flex-1 max-w-sm">
                ...
                <input value={sellerSearch} onChange={e => setSellerSearch(e.target.value)} placeholder="Tìm sản phẩm..."
              ...
              <button onClick={() => setSellerViewMode(v => v === 'grid' ? 'list' : 'grid')}
                className="p-2.5 bg-white border border-gray-200 rounded-2xl hover:bg-gray-50 transition-all text-gray-500">
                    ...
                    return (
                      <div key={p.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition-all group">
                        <div className="h-40 relative bg-gray-50 overflow-hidden">
                          ...
                          {(p.stock||0) > 0 && (p.stock||0) <= 5 && <span className="absolute top-2 right-2 bg-amber-500 text-white text-[9px] font-black px-2 py-0.5 rounded-full">Sắp hết</span>}
                        </div>
                        <div className="p-3">
                          ...
                          <div className="flex items-center justify-between mt-2">
                            ...
                            <div className="flex gap-1">
                              ...
                              <button onClick={() => deleteProduct(p.id)} className="p-1.5 hover:bg-red-50 text-red-500 rounded-lg transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                            </div>
                          </div>
                        </div>
                      </div>
                  ...
                  {filtered.map((p, i) => (
                    <div key={p.id} className={`flex items-center gap-4 p-4 hover:bg-gray-50 transition-colors ${i > 0 ? 'border-t border-gray-50' : ''}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {p.tags?.length ? <div className="flex gap-1 mt-1">{p.tags.slice(0,3).map(t => <span key={t} className="text-[9px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-full font-bold">#{t}</span>)}</div> : null}
                      </div>
                      ...
                      <div className="flex gap-1.5 flex-shrink-0">
                        ...
                        <button onClick={() => deleteProduct(p.id)} className="p-2 hover:bg-red-50 text-red-500 rounded-xl transition-colors"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }}
                    ...
                    <form onSubmit={saveProduct} className="p-6 space-y-5">
                        ...
                        <label className="relative block h-44 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 overflow-hidden cursor-pointer hover:border-blue-400 transition-colors group">
                          {previewUrl ? <Media src={previewUrl} alt="preview" className="w-full h-full" /> : (
                            <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 gap-2">
                              ...
                              <p className="text-xs text-gray-300">Tối đa 10MB</p>
                            </div>
                          )}
                          {previewUrl && <div className="absolute inset-0 bg-black/30 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center"><Camera className="w-8 h-8 text-white" /></div>}
                      ...
                      <div>
                        ...
                        <input required value={productForm.name} onChange={e => setProductForm(p => ({ ...p, name: e.target.value }))} placeholder="Tên sản phẩm..."
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <select value={productForm.category} onChange={e => setProductForm(p => ({ ...p, category: e.target.value, subcategory: '' }))}
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <input type="number" required min="1" value={productForm.price} onChange={e => setProductForm(p => ({ ...p, price: e.target.value }))} placeholder="0"
                            className="w-full bg-gray-50 border border-gray-200 rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                        </div>
                        <div>
                          <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-1.5">Giá gốc (sale)</label>
                          <input type="number" min="0" value={productForm.original_price} onChange={e => setProductForm(p => ({ ...p, original_price: e.target.value }))} placeholder="Để trống nếu không sale"
                            className="w-full bg-gray-50 border border-gray-200 rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                        </div>
                      </div>
                      {/* Stock + Tags */}
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <input value={productForm.tags} onChange={e => setProductForm(p => ({ ...p, tags: e.target.value }))} placeholder="tươi, ngon, sale..."
                            className="w-full bg-gray-50 border border-gray-200 rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                        </div>
                      </div>
                      {/* Description */}
                      <div>
                        ...
                        <textarea rows={3} value={productForm.description} onChange={e => setProductForm(p => ({ ...p, description: e.target.value }))} placeholder="Mô tả chi tiết..."
                          className="w-full bg-gray-50 border border-gray-200 rounded-2xl py-3 px-4 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none" />
                      </div>
                      {/* Actions */}
                      <div className="flex gap-3">
                        ...
                        <button type="submit" disabled={loading} className="flex-[2] py-3 bg-blue-600 hover:bg-blue-700 disabled:opacity-60 text-white font-bold rounded-2xl flex items-center justify-center gap-2 shadow-lg shadow-blue-600/20 text-sm active:scale-[0.98]">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              {['all', ...Object.keys(ORDER_STATUSES)].map(s => (
                <button key={s} onClick={() => setOrderFilter(s)}
                  className={`px-4 py-2 rounded-2xl text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 ${orderFilter === s ? 'bg-gray-900 text-white' : 'bg-white border border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                    ...
                    <div key={order.id} className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-md transition-shadow">
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        ...
                        {(order.status === 'Chờ xác nhận' || order.status === 'Đã xác nhận') && (
                          <button onClick={() => cancelOrder(order)} className="px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-600 text-xs font-bold rounded-xl transition-colors">Huỷ đơn</button>
                        )}
                      </div>
                    </div>
                  );
          ...
          <motion.main key="marketplace" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="bg-gradient-to-r from-red-600 to-orange-500 rounded-2xl p-4 flex items-center justify-between">
              <div>
                ...
                <p className="text-white/80 text-xs font-medium">Dùng mã: <span className="font-black text-yellow-300">SALE10</span>, <span className="font-black text-yellow-300">WELCOME</span>, <span className="font-black text-yellow-300">FIT20</span>, <span className="font-black text-yellow-300">VIP30</span></p>
              </div>
            ...
            <div className="relative flex gap-2">
              <div className="relative flex-1">
                ...
                <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Tìm sản phẩm, danh mục, tags..."
                  className="w-full bg-white border border-gray-200 rounded-2xl py-3.5 pl-12 pr-10 text-sm focus:outline-none focus:ring-4 focus:ring-blue-500/10 shadow-sm" />
                {searchQuery && <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-gray-100 rounded-lg transition-colors"><X className="w-4 h-4 text-gray-400" /></button>}
              </div>
              <button onClick={() => setShowFilters(!showFilters)}
                className={`px-4 py-3.5 rounded-2xl border-2 transition-all text-sm font-bold flex items-center gap-2 ${showFilters ? 'bg-blue-600 text-white border-blue-600 shadow-lg' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'}`}>
                <Filter className="w-4 h-4" /><span className="hidden sm:block">Bộ lọc</span>
              </button>
              {hidden.length > 0 && (
                <button onClick={() => setShowHidden(true)} className="px-4 py-3.5 rounded-2xl border-2 border-gray-200 bg-white text-gray-500 hover:border-gray-300 transition-all text-sm font-bold flex items-center gap-2" title="Sản phẩm đã ẩn">
                  <EyeOff className="w-4 h-4" /><span className="hidden sm:block">{hidden.length}</span>
                </button>
              )}
            </div>
                  ...
                  <div className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm grid grid-cols-1 sm:grid-cols-3 gap-6">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        ...
                        {(priceRange[0] !== 0 || priceRange[1] !== 10_000_000) && <button onClick={() => setPriceRange([0,10_000_000])} className="text-xs text-blue-500 font-bold">✕ Xoá bộ lọc giá</button>}
                      </div>
                    </div>
                    <div>
                      ...
                      <button onClick={() => { setSearchQuery(''); setSelectedCat('all'); setSelectedSub(''); setPriceRange([0,10_000_000]); setSortBy('newest'); setShowFilters(false); }}
                        className="w-full py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 font-bold rounded-xl text-sm transition-colors">Đặt lại bộ lọc</button>
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
              {CATEGORIES.map(cat => (
                <button key={cat.id} onClick={() => { setSelectedCat(cat.id); setSelectedSub(''); }}
                  className={`flex items-center gap-1.5 px-3.5 py-2 rounded-2xl text-sm font-bold whitespace-nowrap transition-all flex-shrink-0 ${selectedCat === cat.id ? 'bg-gray-900 text-white shadow-lg' : 'bg-white border border-gray-200 text-gray-600 hover:border-gray-300'}`}>
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500">
                {loading ? 'Đang tải...' : <><span className="font-bold text-gray-900">{filteredProducts.length}</span> sản phẩm{searchQuery && <span className="text-blue-600"> cho "<strong>{searchQuery}</strong>"</span>}{hidden.length > 0 && <span className="text-gray-400"> (đã ẩn {hidden.length})</span>}</>}
              </p>
            ...
            {/* Products */}
            {loading ? (
              <div className="flex items-center justify-center h-64 gap-3"><Loader2 className="w-8 h-8 animate-spin text-blue-600" /><span className="text-gray-400">Đang tải sản phẩm...</span></div>
                ...
                <AnimatePresence>
                  {filteredProducts.map(p => (
                    <ProductCard key={p.id} product={p} onAdd={addToCart} onView={p => { setViewProduct(p); fetchReviews(p.id); }}
                      ...
                      hidden={hidden.includes(p.id)} onHide={id => { const prod = allProducts.find(x => x.id === id); if (prod) setPendingHide(prod); }} />
                  ))}
                </AnimatePresence>
              </motion.div>
            ) : (
        ...
        {activeTab === 'my-purchases' && (
          <motion.main key="my-purchases" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between">
              ...
              <button onClick={fetchBuyerOrders} className="flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 rounded-2xl text-sm font-bold hover:bg-gray-50 shadow-sm">
                <RefreshCw className="w-4 h-4" />Làm mới
              </button>
            </div>
        ...
        {activeTab === 'wishlist-tab' && (
          <motion.main key="wishlist-tab" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between">
              ...
              {hidden.length > 0 && (
                <button onClick={() => setShowHidden(true)} className="flex items-center gap-2 px-4 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-2xl text-sm font-bold transition-all">
                  <EyeOff className="w-4 h-4" />{hidden.length} đã ẩn
                </button>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {wishlistProducts.map(p => (
                  <ProductCard key={p.id} product={p} onAdd={addToCart}
                    onView={p => { setViewProduct(p); fetchReviews(p.id); }}
                  ...
                  {cart.length > 0 && (
                    <div className="p-4 border-t border-gray-100 space-y-3 bg-white flex-shrink-0">
                      ...
                      {couponDiscount > 0 ? (
                        ...
                        <div className="flex gap-2">
                          <input value={couponCode} onChange={e => setCouponCode(e.target.value.toUpperCase())} placeholder="Mã giảm giá..." onKeyDown={e => e.key === 'Enter' && applyCoupon()}
                ...
                <>
                  <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    <div>
                      ...
                      <textarea value={shippingAddr} onChange={e => setShippingAddr(e.target.value)} rows={3} placeholder="Số nhà, tên đường, phường/xã, quận/huyện, tỉnh/thành..."
                        className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none" />
                    </div>
                    <div>
                      <label className="text-xs font-black text-gray-400 uppercase tracking-widest block mb-2">Ghi chú (tuỳ chọn)</label>
                      <textarea value={orderNote} onChange={e => setOrderNote(e.target.value)} rows={2} placeholder="Thời gian giao, yêu cầu đặc biệt..."
                        className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 resize-none" />
                    </div>
                  </div>
                  <div className="p-4 border-t border-gray-100 flex-shrink-0">
                    <button onClick={() => { if (!shippingAddr.trim()) { toast('Vui lòng nhập địa chỉ', 'error'); return; } setCheckoutStep('payment'); }}
          ...
          {role === 'buyer' ? (
            ...
            <>
              {[{ id: 'my-products', icon: Store, label: 'Sản phẩm' }, { id: 'my-orders', icon: BarChart3, label: 'Đơn hàng' }].map(({ id, icon: Icon, label }) => (
                <button key={id} onClick={() => setActiveTab(id)} className={`flex flex-col items-center gap-0.5 px-6 py-2 rounded-xl transition-all ${activeTab === id ? 'text-blue-600' : 'text-gray-400'}`}>
                  <Icon className="w-5 h-5" /><span className="text-[9px] font-bold">{label}</span>
                </button>
              ))}
              <button onClick={() => setShowProductForm(true)} className="flex flex-col items-center gap-0.5 px-6 py-2 text-gray-400">
                <PackagePlus className="w-5 h-5" /><span className="text-[9px] font-bold">Thêm SP</span>
              </button>
              <button onClick={() => setShowNotifications(true)} className="flex flex-col items-center gap-0.5 px-6 py-2 text-gray-400 relative">
                <Bell className="w-5 h-5" />
